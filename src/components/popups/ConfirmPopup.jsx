...
export default function ConfirmPopup() {
  ...
  const handleDeletePost=(option)=>{
   ...
   axios.delete(`${apiUrl}/posts/${confirmPopup.id}`)
   .then((res)=>{
    console.log(res)
    setConfirmPopup({state:false,id:""})
    setNotificationPopup({state:true,text:"Post deleted successfully"})
   })
   .catch((err)=>{
    console.log(err)
  ...
  return (
    <BootstrapDialog open={confirmPopup.state} onClose={onClosePopup}>
       ...
       <Button onClick={()=>handleDeletePost(false)} sx={{minWidth:"100px",marginX:"5px",marginY:"20px"}} variant="contained" color='error'>No</Button> 
       <Button onClick={()=>handleDeletePost(true)} sx={{minWidth:"100px",marginX:"5px",marginY:"20px"}} variant="contained" color='warning'>Yes</Button> 
       </Box>
    </BootstrapDialog>
  )
}
